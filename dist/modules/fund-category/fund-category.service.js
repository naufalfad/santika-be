...
class FundCategoryService {
     ...
     */
    static async transferBalance(parokiId, userId, input) {
        if (input.sourceFundCategoryId === input.targetFundCategoryId) {
            throw api_error_1.ApiError.badRequest('Pos Dana asal dan tujuan tidak boleh sama');
        }
        if (input.amount <= 0) {
            throw api_error_1.ApiError.badRequest('Nominal transfer harus lebih dari 0');
        }
        // Verify Source Fund Category
        const sourceFund = await database_1.prisma.fundCategory.findUnique({
            where: { id: input.sourceFundCategoryId },
            include: { specialFund: true },
        });
        if (!sourceFund) {
            throw api_error_1.ApiError.notFound('Pos Dana asal tidak ditemukan');
        }
        if (sourceFund.parokiId !== parokiId) {
            throw api_error_1.ApiError.forbidden('Pos Dana asal berada di luar paroki Anda');
        }
        if (!sourceFund.isActive) {
            throw api_error_1.ApiError.badRequest('Pos Dana asal tidak aktif');
        }
        if (sourceFund.specialFund) {
            throw api_error_1.ApiError.badRequest('Tidak dapat mentransfer saldo langsung dari Pos Dana khusus. Gunakan modul alokasi sisa dana khusus.');
        }
        // Verify Target Fund Category
        const targetFund = await database_1.prisma.fundCategory.findUnique({
        ...
        if (targetFund.parokiId !== parokiId) {
            throw api_error_1.ApiError.forbidden('Pos Dana tujuan berada di luar paroki Anda');
        }
        if (!targetFund.isActive) {
            throw api_error_1.ApiError.badRequest('Pos Dana tujuan tidak aktif');
        }
        if (targetFund.specialFund) {
            throw api_error_1.ApiError.badRequest('Tidak dapat mentransfer saldo langsung ke Pos Dana khusus.');
        }
        ...
        if (currentBalance < input.amount) {
            throw api_error_1.ApiError.badRequest(`Saldo ${sourceFund.name} tidak mencukupi. Saldo tersedia: Rp ${currentBalance.toLocaleString('id-ID')}, Jumlah transfer: Rp ${input.amount.toLocaleString('id-ID')}`);
        }
        // Find the default income type and expense type for transfer
        const firstIncomeType = await database_1.prisma.incomeType.findFirst({
        ...
        const firstExpenseType = await database_1.prisma.expenseType.findFirst({
            where: { parokiId, isActive: true },
        });
        if (!firstExpenseType) {
