...
class ReportService {
     ...
     */
    static getPeriodDateRange(periodStr) {
        let year;
        let month;
        if (periodStr) {
     ...
     * 1. Buku Kas Umum (BKU)
     */
    static async getBkuReport(parokiId, periodStr, search) {
        const { startDate, endDate } = this.getPeriodDateRange(periodStr);
        // Calculate starting balance before startDate
        const incomeBefore = await database_1.prisma.cashTransaction.aggregate({
            where: {
                parokiId,
        ...
        const expenseBefore = await database_1.prisma.cashTransaction.aggregate({
            where: {
                parokiId,
        ...
        if (search) {
            ...
            filteredRecords = allRecords.filter((rec) => rec.keterangan.toLowerCase().includes(searchLower) ||
     ...
     * 2. Arus Kas
     */
    static async getCashFlowReport(parokiId, periodStr) {
        const { startDate, endDate } = this.getPeriodDateRange(periodStr);
        const transactions = await database_1.prisma.cashTransaction.findMany({
            where: {
                parokiId,
        ...
        transactions.forEach((tx) => {
            ...
            if (tx.transactionType === 'INCOME') {
                ...
                const isOps = catCode === 'OPERASIONAL' ||
                    [
                        'LISTRIK',
                        'AIR',
                        'INTERNET',
     ...
     */
    static async getBudgetRealisationReport(parokiId, yearVal) {
